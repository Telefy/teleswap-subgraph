...
export function createLiquiditySnapshot(position: LiquidityPosition, event: ethereum.Event): void {
  ...
  if (pair) {
    ...
    if (token1 && token0) {
      ...
      const tk0deth = token0.derivedETH;
      const tk1deth = token1.derivedETH;
      if (tk0deth && tk1deth && bundle) {
        snapshot.token0PriceUSD = tk0deth.times(bundle.ethPrice)
        snapshot.token1PriceUSD = tk1deth.times(bundle.ethPrice)
      }
